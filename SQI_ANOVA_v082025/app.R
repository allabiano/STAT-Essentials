...
# install.packages(c("shiny", "shinydashboard", "readr", "dplyr", "FactoMineR", "ggplot2", "DT", "forcats", "tibble", "tidyr", "factoextra", "writexl", "ggcorrplot", "agricolae", "ggpattern"))
...
library(ggcorrplot)
library(agricolae)
library(ggpattern) # New library for patterns
...
server <- function(input, output, session) {
  ...
  sqi_group_summary_table <- reactive({
    ...
    summary_data <- calculated_sqi_data()$sqi_data %>%
  ...
  output$sqi_group_bar_plot <- renderPlot({
    req(calculated_sqi_data()$sqi_data, input$group_var_for_plot)
    ...
    plot_data_full <- calculated_sqi_data()$sqi_data
    
    if (n_distinct(plot_data_full[[group_var]]) <= 1) {
      return(
        ggplot() +
          geom_text(aes(x = 0.5, y = 0.5, label = "The selected grouping variable has only one level. A grouped bar chart cannot be generated.")) +
          theme_void() +
          labs(title = "SQI Grouped Bar Graph")
      )
    }
    ...
    plot_data <- plot_data_full %>%
      ...
      mutate(
        Quality = case_when(
          Mean_SQI >= 80 ~ "Q1: Very High",
          Mean_SQI >= 60 ~ "Q2: High",
          Mean_SQI >= 40 ~ "Q3: Moderate",
          Mean_SQI >= 20 ~ "Q4: Low",
          TRUE ~ "Q5: Very Low"
        ),
        Quality = factor(Quality, levels = c("Q1: Very High", "Q2: High", "Q3: Moderate", "Q4: Low", "Q5: Very Low"))
      )
    
    formula_aov <- reformulate(termlabels = group_var, response = "SQI")
    aov_model <- aov(formula_aov, data = plot_data_full)
    p_value <- summary(aov_model)[[1]][["Pr(>F)"]][1]
    
    if (!is.na(p_value) && p_value < 0.05) {
      tukey_result <- HSD.test(aov_model, trt = group_var, group = TRUE, console = FALSE)
      cld_df <- as.data.frame(tukey_result$groups) %>%
        rownames_to_column(var = group_var) %>%
        rename(CLD = groups)
      
      plot_data <- plot_data %>%
        left_join(cld_df, by = group_var) %>%
        mutate(CLD = if_else(is.na(CLD), "", CLD))
      
      cld_plot <- plot_data %>%
        mutate(y_pos = Mean_SQI + SD_SQI + 2)
      
      # Define a custom black and white theme with patterns
      sqi_bw_fill <- c(
        "Q1: Very High" = "gray10",
        "Q2: Good" = "gray30",
        "Q3: Moderate" = "gray50",
      ...
      # Define patterns to be used for the categories
      sqi_patterns <- c(
        "Q1: Very High" = "crosshatch",
        "Q2: High" = "stripe",
        "Q3: Moderate" = "pch",
        "Q4: Low" = "none",
        "Q5: Very Low" = "circle"
      ...
      p <- ggplot(plot_data, aes(x = !!sym(group_var), y = Mean_SQI, fill = Quality)) +
        geom_bar_pattern(
          aes(pattern = Quality),
          stat = "identity", 
          color = "black", 
        ...
        geom_errorbar(aes(ymin = Mean_SQI - SD_SQI, ymax = Mean_SQI + SD_SQI), width = 0.2) +
        scale_fill_manual(values = sqi_bw_fill, name = "SQI Quality") +
        scale_pattern_manual(values = sqi_patterns, name = "SQI Quality") +
        
        geom_text(data = cld_plot, aes(x = !!sym(group_var), y = y_pos, label = CLD),
                  vjust = 0, size = 5, fontface = "bold") +
        
        labs(
          title = paste("Mean Soil Quality Index by", group_var, "with Tukey's HSD Test"),
      ...
      # Plot without CLD, but still with the new B&W theme
      sqi_bw_fill <- c(
        "Q1: Very High" = "gray10",
        "Q2: High" = "gray30",
        "Q3: Moderate" = "gray50",
      ...
      p <- ggplot(plot_data, aes(x = !!sym(group_var), y = Mean_SQI, fill = Quality)) +
        ...
        scale_fill_manual(values = sqi_bw_fill, name = "SQI Quality") +
        labs(
          title = paste("Mean Soil Quality Index by", group_var),
          subtitle = "ANOVA was not significant (p >= 0.05). No CLD generated.",
          x = group_var,
          y = "Mean Soil Quality Index (0-100)"
        ) +
        theme_classic() +
